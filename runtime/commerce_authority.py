...
class CommerceBlocked(RuntimeError):
    pass


class CommerceInvalid(ValueError):
    pass
...
def normalize_address(value: str) -> str:
    ...
    if not (v.startswith("0x") and len(v) == 42):
        raise CommerceInvalid("invalid ethereum address")
    return v.lower()
...
def quote_hash(quote_without_hash: dict[str, Any]) -> str:
...
def build_quote(
    *,
    ...
    if not sku or not nonce or not policy_version:
        raise CommerceInvalid("sku, nonce and policy_version are required")
    parse_time(expires_at)
    body = {
        "schema": "janus.machine_market.quote.v1",
        "sku": sku,
        "request_hash": request_hash(request),
        "amount_usdt_micros": int(amount_usdt_micros),
        "asset": "USDT",
        "chain_id": CHAIN_ID,
        "token_contract": USDT_ETHEREUM,
        "receiving_address": normalize_address(receiving_address),
...
def verify_quote(quote: dict[str, Any], request: dict[str, Any], *, now: datetime | None = None) -> None:
    q = dict(quote)
    supplied_hash = q.pop("quote_hash", None)
    if not supplied_hash or supplied_hash != quote_hash(q):
        raise CommerceInvalid("quote hash mismatch")
    if q.get("request_hash") != request_hash(request):
        raise CommerceInvalid("request hash mismatch")
    if q.get("asset") != "USDT":
        raise CommerceInvalid("unexpected asset")
    if q.get("chain_id") != CHAIN_ID:
        raise CommerceInvalid("unsupported chain")
    if normalize_address(q.get("token_contract")) != normalize_address(USDT_ETHEREUM):
        raise CommerceInvalid("unexpected token contract")
    if int(q.get("amount_usdt_micros", -1)) < 0:
        raise CommerceInvalid("invalid amount")
    if parse_time(q["expires_at"]) <= (now or utc_now()):
...
def verify_payment_receipt(
    ...
    *,
    ...
    if int(receipt.get("chain_id", -1)) != int(quote["chain_id"]):
        raise CommerceInvalid("payment chain mismatch")
    if normalize_address(receipt.get("token_contract")) != normalize_address(quote["token_contract"]):
        raise CommerceInvalid("payment token mismatch")
    if normalize_address(receipt.get("to")) != normalize_address(quote["receiving_address"]):
    ...
    if int(receipt.get("confirmations", 0)) < MIN_CONFIRMATIONS:
        raise CommerceInvalid("payment confirmation threshold not met")
...
def admit_purchase(
    *,
    ...
    if foreign_witness.get("foreign_agent_witness") is not True:
        raise CommerceBlocked("foreign agent witness is not established")
    if product.get("machine_purchase") is not True:
        raise CommerceBlocked("product is not machine-purchasable")
    if quote.get("sku") != product.get("sku"):
        raise CommerceInvalid("quote/product SKU mismatch")
