...
from runtime.commerce_authority import CommerceBlocked, CommerceInvalid, USDT_ETHEREUM, admit_purchase, build_quote
...
def request(): return {"schema": "janus.machine_market.request.v1", "sku": "JANUS.SEARCH", "input": {"query": "test"}}
def quote(): return build_quote(request=request(), sku="JANUS.SEARCH", amount_usdt_micros=50_000, receiving_address=RECEIVER, expires_at="2026-08-31T19:00:00+00:00", nonce="nonce-1", policy_version="commerce-v1")
def receipt(amount=50_000, confirmations=12, required_confirmations=12, log_index=LOG_INDEX, block_timestamp="2026-08-31T18:59:00+00:00"):
    q=quote(); return {"schema":"janus.machine_market.payment_receipt.v1","status":"CONFIRMED","quote_hash":q["quote_hash"],"tx_hash":TX,"log_index":log_index,"payment_reference":f"{TX}:{log_index}","chain_id":1,"token_contract":USDT_ETHEREUM,"to":RECEIVER,"amount_usdt_micros":amount,"confirmations":confirmations,"required_confirmations":required_confirmations,"block_timestamp":block_timestamp}
...
def test_payment_mined_before_expiry_can_settle_after_quote_expired():
    grant=admit_purchase(readiness=live_readiness(),foreign_witness=witness(True),product=product(True),request=request(),quote=quote(),payment_receipt=receipt(),now=NOW)
    assert grant["status"]=="PURCHASE_SETTLED"
    assert grant["payment_reference"]==PAYMENT_REF


def test_payment_mined_after_quote_expiry_is_rejected():
    with pytest.raises(CommerceInvalid, match="mined after quote expiry"):
        admit_purchase(readiness=live_readiness(),foreign_witness=witness(True),product=product(True),request=request(),quote=quote(),payment_receipt=receipt(block_timestamp="2026-08-31T19:00:01+00:00"),now=NOW)
...
def test_exact_payment_and_open_product_produce_deterministic_purchase_grant():
    ...
    assert a["status"]=="PURCHASE_SETTLED"
    assert a["execution_authority_granted"] is False
    assert a["next_gate"]=="JANUS_POLICY_SCOPE_TO_EXECUTION_GRANT"


def test_paid_search_buyer_entitlement_is_bounded_and_deterministic():
    kwargs=dict(readiness=live_readiness(),foreign_witness=witness(True),product=product(True),request=request(),quote=quote(),payment_receipt=receipt(),now=NOW,buyer_actor_id="github:external-buyer")
    a=admit_purchase(**kwargs); b=admit_purchase(**kwargs)
    assert a==b
    ent=a["buyer_query_entitlement"]
    assert a["allowed_operation"]=="REQUEST_BOUNDED_BUYER_QUERY"
    assert a["next_gate"]=="JANUS_HOME_READ_ONLY_BUYER_QUERY"
    assert ent["buyer_actor_id"]=="github:external-buyer"
    assert ent["max_turns"]==1
    assert ent["max_message_utf8_bytes"]==4000
    assert ent["max_answer_utf8_bytes"]==6000
    assert ent["conversation_history_turns"]==0
    assert ent["read_only_conversation"] is True
    assert ent["external_effect_authorized"] is False
