...
def request():
    return {"schema": "janus.machine_market.request.v1", "sku": "JANUS.SEARCH", "input": {"query": "ledger"}}


def quote():
    return build_quote(request=request(), sku="JANUS.SEARCH", amount_usdt_micros=100_000, receiving_address=RECEIVER, expires_at="2026-08-31T19:00:00+00:00", nonce="ledger-nonce", policy_version="v1")
...
def grant():
    return admit_purchase(
        ...
        request=request(), quote=quote(), payment_receipt=receipt(), now=NOW,
    )


def test_create_then_exact_retry_is_idempotent(tmp_path):
    a = persist_purchase(tmp_path, receipt(), grant())
    b = persist_purchase(tmp_path, receipt(), grant())
    assert a["payment"] == "CREATED"
    assert a["purchase"] == "CREATED"
    assert b["payment"] == "IDEMPOTENT_REPLAY"
    assert b["purchase"] == "IDEMPOTENT_REPLAY"
...
def test_same_payment_reference_cannot_point_to_conflicting_purchase(tmp_path):
    persist_purchase(tmp_path, receipt(), grant())
    bad = dict(grant())
    bad["purchase_id"] = "jp-conflict"
    with pytest.raises(LedgerConflict):
        persist_purchase(tmp_path, receipt(), bad)
...
def test_corrupt_persistent_payment_record_fails_closed(tmp_path):
    p = tmp_path / "state/commerce/payments/bad.json"
    p.parent.mkdir(parents=True)
    p.write_text("not-json", encoding="utf-8")
    with pytest.raises(LedgerConflict, match="invalid persistent payment ledger"):
        consumed_payment_references(tmp_path)
